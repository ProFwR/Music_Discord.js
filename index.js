...
client.on('ready', () => {
  ...
  function msg() {
    let status = [`${prefix}help | Users ${client.guilds.cache
          .reduce((a, b) => a + b.memberCount, 0)
          .toLocaleString()}`];
...
//=================================
    ...
    if (message.author.bot) return;
      ...
      .join(' ');
		...
		return message.lineReplyNoMention(new Discord.MessageEmbed()                    .setDescription(`You must join a voice channel to use that!`)           .setColor(Em_color)                        );
if (message.guild.me.voice.channel && message.guild.me.voice.channel.id !== message.member.voice.channel.id) return message.lineReplyNoMention(new Discord.MessageEmbed()                       .setDescription(`You must be listening in <#${message.guild.me.voice.channel.id}> to use that!`)                       .setColor(Em_color)
    ...
    if (message.author.bot) return;
        if (!message.member.voice.channel)
            ...
            if (message.guild.me.voice.channel && message.guild.me.voice.channel.id !== message.member.voice.channel.id) 
              	return message.lineReplyNoMention(new Discord.MessageEmbed()                    .setDescription(`You must be listening in <#${message.guild.me.voice.channel.id}> to use that!`)           .setColor(Em_color)                        );
	const queue = client.distube.getQueue(message)
        if (!queue)
          return message.lineReplyNoMention(new Discord.MessageEmbed()                    .setDescription(`The player has stopped and the queue has been cleared`)           .setColor(Em_color)                       );      message.member.voice.channel.leave()
		client.distube.stop(message)
	    return message.lineReplyNoMention(new Discord.MessageEmbed()                     .setDescription(`The song has been successfully Stoped`)           .setColor(Em_color)                       );
	}
...
//=================================
    ...
    if (message.author.bot) return;
        if (!message.member.voice.channel)
            ...
            if (message.guild.me.voice.channel && message.guild.me.voice.channel.id !== message.member.voice.channel.id) 				return message.lineReplyNoMention(new Discord.MessageEmbed()                    .setDescription(`You must be listening in <#${message.guild.me.voice.channel.id} >to use that!`)           .setColor(Em_color)                        );
const queue = client.distube.getQueue(message)
        if (!queue) 	return message.lineReplyNoMention(new Discord.MessageEmbed()                    .setDescription(`The player has stopped and the queue has been cleared.`)           .setColor(Em_color)                        ), message.member.voice.channel.leave()
		client.distube.skip(message);
	return message.lineReplyNoMention(new Discord.MessageEmbed()                  .setDescription(`The song has been successfully Skiped`)           .setColor(Em_color)                        );
	}
...
const args = message.content.split(" ")
        if (!message.member.voice.channel)
            ...
            if (message.guild.me.voice.channel && message.guild.me.voice.channel.id !== message.member.voice.channel.id) 
              return message.lineReplyNoMention(new Discord.MessageEmbed()                    .setDescription(`You must be listening in <#${message.guild.me.voice.channel.id} >to use that!`)           .setColor(Em_color)                        );
const queue = client.distube.getQueue(message)
        if (!queue)   return message.lineReplyNoMention(new Discord.MessageEmbed()                 .setDescription(`There is nothing in the queue right now!`)           .setColor(Em_color)                        );
        let mode = null
        switch (args[0]) {
            case "off":
                mode = 0
        ...
        mode = mode ? mode === 2 ? "repeat queue" : "repeat song" : "off"
	       return message.lineReplyNoMention(new Discord.MessageEmbed()                 .setDescription(`Set repeat mode to ${mode}`)           .setColor(Em_color)                        );
	}
...
//=================================
    ...
    if (message.author.bot) return;
		...
		if(!queue) return message.lineReplyNoMention(new Discord.MessageEmbed()                    .setDescription(`There is nothing in the queue right now!`)           .setColor(Em_color)                        );		const embed = new Discord.MessageEmbed()
  ...
  .setColor(Em_color)    .setDescription(queue.songs.map((song, id) =>
...
//=================================
    ...
    if (message.author.bot) return;
            ...
            if (message.guild.me.voice.channel && message.guild.me.voice.channel.id !== message.member.voice.channel.id) return message.lineReplyNoMention(new Discord.MessageEmbed()                    .setDescription(`You must be listening in <#${message.guild.me.voice.channel.id} >to use that!`)           .setColor(Em_color)                        );
const args = message.content.split(" ")
if(!args) return message.lineReplyNoMention(new Discord.MessageEmbed()                    .setDescription(`Please type number to set volume!`)           .setColor(Em_color)                        ); 
const queue = client.distube.getQueue(message)
        if (!queue) return message.lineReplyNoMention(new Discord.MessageEmbed()                    .setDescription(`There is nothing in the queue right now!`)           .setColor(Em_color)                        );
  const volume = parseInt(args[1])
if (isNaN(volume)) return message.lineReplyNoMention(new Discord.MessageEmbed()                    .setDescription(`Please enter a valid number!`)           .setColor(Em_color)                        );
if(volume > 100) return message.lineReplyNoMention(new Discord.MessageEmbed()                    .setDescription(`The Max Volume is 100`)
   .setColor(Em_color)                      );	    client.distube.setVolume(message, volume)
return message.lineReplyNoMention(new Discord.MessageEmbed()                    .setDescription(`Volume set to ${volume}`)           .setColor(Em_color)                      );
	}

//=================================

	if (message.content.startsWith(prefix + "pause")) {
        if (!message.member.voice.channel) return message.lineReplyNoMention(new Discord.MessageEmbed()                    .setDescription(`You must join a voice channel to use that!`)           .setColor(Em_color)                        );
            if (message.guild.me.voice.channel && message.guild.me.voice.channel.id !== message.member.voice.channel.id) return message.lineReplyNoMention(new Discord.MessageEmbed()                    .setDescription(`You must be listening in <#${message.guild.me.voice.channel.id} >to use that!`)           .setColor(Em_color)                        );
const queue = client.distube.getQueue(message)
        if (!queue) return message.lineReplyNoMention(new Discord.MessageEmbed()                    .setDescription(`There is nothing in the queue right now!`)           .setColor(Em_color)                        );
if (queue.pause) {
return message.lineReplyNoMention(new Discord.MessageEmbed()                    .setDescription(`The player is already paused! Use \`${prefix}resume\` to unpause!`)           .setColor(Em_color)                        )            
        }
...
message.lineReplyNoMention(new Discord.MessageEmbed()                       .setDescription(`Can't find related video to play. Stop playing music.`)                       .setColor(Em_color)
 ))
